...
<Seo
	title={`${userProfile?.displayUsername || userProfile?.username || 'User'}'s Profile - Quizify`}
	description={`Check out ${userProfile?.displayUsername || userProfile?.username || 'User'}'s profile, stats, and quizzes on Quizify.`}
	url={page.url.href}
	image={userProfile?.image || '/logo.png'}
	type="profile"
	keywords={`quizify, profile, ${userProfile?.username}, user stats, user quizzes`}
/>
